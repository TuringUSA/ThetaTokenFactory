...
import { tokenFactory } from "./theta-token-factory.contract";
import { ethers } from "ethers";

describe("ThetaTokenFactory", function () {
  context("IAM", async () => {
    it("init owner", async () => {
    ...
    it("revert if not owner try withdraw native tokens", async () => {
      ...
      await expect(
        contract.connect(alice).withdrawTFuel(alice.address)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
  ...
  context("Create token", async () => {
    ...
    it("reverts if user try create token and didn't pay for it", async () => {
      ...
      await expect(
        contract.connect(alice).createToken("Token", "TFUEL", 100000)
